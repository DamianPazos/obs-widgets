<script lang="ts">
  ...
  /**
   * Editor reutilizable de un widget: formulario de parámetros + editor de
   * posiciones (vista previa interactiva). Trabaja sobre un mapa de params
   * "disperso" (solo overrides respecto del default) más una clave `layout`.
   *
   * Lo usan tanto el panel de Personalizar (`ConfigPanel`) como el constructor
   * de escenas (`Builder`), donde cada instancia guarda sus propios params.
   */
  let {
    widgetId,
    ...
    widgetId: string;
    /** Params iniciales (dispersos: solo overrides + opcional `layout`). */
    params?: Record<string, string>;
    /** Se llama con los params dispersos cada vez que cambian. */
    onChange?: (params: Record<string, string>) => void;
  } = $props();

  // widgetId es estable durante la vida del componente (se remonta por instancia
  // con {#key} en el Builder, y es fijo en el panel). Capturar el valor inicial
  // es intencional.
  // svelte-ignore state_referenced_locally
  const widget = getWidget(widgetId);
  const decl: WidgetParam[] = widget?.params ?? [];
  const elements = widget?.elements ?? [];

  function defaults(): Record<string, string> {
    const out: Record<string, string> = {};
    for (const p of decl) out[p.name] = String(p.default);
    return out;
  }

  // Valores efectivos (default + overrides) para poblar el formulario. Solo
  // tomamos el valor inicial de los params: a partir de ahí, el estado es propio.
  // svelte-ignore state_referenced_locally
  let values = $state<Record<string, string>>({ ...defaults(), ...stripLayout(initialParams) });
  // El layout se maneja aparte como string codificado.
  // svelte-ignore state_referenced_locally
  let layoutStr = $state<string>(initialParams.layout ?? '');
  ...
  const layoutMap = $derived<Layout>(decodeLayout(layoutStr || null));
  const selPoint = $derived(
    selectedId ? (layoutMap[selectedId] ?? ({ x: 50, y: 50 } as Point)) : null,
  );

  function stripLayout(p: Record<string, string>): Record<string, string> {
    const rest: Record<string, string> = {};
    for (const [k, v] of Object.entries(p)) if (k !== 'layout') rest[k] = v;
    return rest;
  }
  ...
  // Params dispersos = solo overrides respecto del default + layout.
  function buildSparse(): Record<string, string> {
    const out: Record<string, string> = {};
    for (const p of decl) {
    ...
    if (layoutStr) out.layout = layoutStr;
    return out;
  }

  // Notificamos al padre en cada cambio de valores o layout.
  $effect(() => {
    void JSON.stringify(values);
    void layoutStr;
    untrack(() => onChange?.(buildSparse()));
  });

  // Vista previa (edición): params no-default + flags. El layout NO va en la URL
  // (se sincroniza por postMessage para no recargar el iframe en cada arrastre).
  const previewUrl = $derived.by(() => {
    const usp = new URLSearchParams({ widget: widgetId });
    for (const p of decl) {
      const v = values[p.name];
      if (v == null || v === '' || v === String(p.default)) continue;
      usp.set(p.name, v);
  ...
  function onIframeLoad(): void {
    sendToWidget({ type: 'set-layout', value: layoutStr });
  ...
  $effect(() => {
    function onMessage(event: MessageEvent): void {
      ...
      if (data.type === 'ready') {
        ...
        layoutStr = data.value;
      } else if (data.type === 'select' && typeof data.id === 'string') {
        selectedId = data.id;
        if (data.pos) {
          const next = { ...layoutMap, [data.id]: data.pos };
          layoutStr = encodeLayout(next);
        }
      }
    }
  ...
  function updateSelected(patch: Partial<Point>): void {
    ...
    const next = { ...layoutMap, [selectedId]: { ...cur, ...patch } };
    layoutStr = encodeLayout(next);
    sendToWidget({ type: 'set-layout', value: layoutStr });
  }
...
{#if !widget}
  <p class="wc-missing">No existe un widget con id <code>{widgetId}</code>.</p>
{:else}
  <div class="wc-layout">
    {#if selectedId}
          ...
          <div class="field">
            ...
            {:else if param.type === 'image'}
              <div class="image-field">
                ...
                />
                {#if values[param.name]}
                  <button type="button" class="clear-img" onclick={() => (values[param.name] = '')}>
                    Quitar imagen
                  </button>
                {/if}
              </div>
